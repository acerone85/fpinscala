...
object MyModule {
  ...
  def fib(n: Int): Int = {
    ...
    @tailrec
    def fibAux(m: Int, secondLastFib: Int, lastFib: Int): Int = {
      if (m == n) secondLastFib
      else fibAux(m+1, lastFib, secondLastFib + lastFib)
    }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    ...
    @tailrec
    def isSortedFromIndex(n: Int): Boolean = {
      if (n >= as.length - 1) true
      else !gt(as(n), as(n+1)) && isSortedFromIndex(n+1)
    }
  ...
  def main(args: Array[String]): Unit = {
    println(isSorted(Array.emptyIntArray, (x: Int, y: Int) => x > y ))
    println(isSorted(Array[Int](1), (x: Int, y: Int) => x > y ))
    println(isSorted(Array[Int](1,2,3,4,5), (x: Int, y: Int) => x > y ))
    println(isSorted(Array[Int](1,1,1,1), (x: Int, y: Int) => x > y))
    println(isSorted(Array[Int](1,2,1), (x: Int, y: Int) => x > y))
  }
